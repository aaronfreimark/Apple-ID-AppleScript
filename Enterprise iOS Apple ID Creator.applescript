...
on ProvidePaymentDetails(userFirstName, userLastName, addressStreet, addressCity, addressState, addressZip, phoneAreaCode, phoneNumber)
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		...
		if pageVerification is "Verified" then
			...
			tell application "System Events"
				set theForm to UI element 1 of scroll area 1 of splitter group 1 of splitter group 1 of window 1 of application process "iTunes"
				if (count of static text of theForm) ≥ 2 then set errorList to errorList & value of static text 2 of theForm
				if (count of static text of theForm) ≥ 3 then set errorList to errorList & value of static text 3 of theForm
				if (count of static text of theForm) ≥ 4 then set errorList to errorList & value of static text 4 of theForm
				if (count of static text of theForm) ≥ 5 then set errorList to errorList & value of static text 5 of theForm
				if (count of static text of theForm) ≥ 6 then set errorList to errorList & value of static text 6 of theForm
			end tell
		end if
