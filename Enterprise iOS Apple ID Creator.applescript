		...
		if scriptAction is "Continue" then
			...
			if scriptAction is not "Abort" then
				...
				repeat with loopCounter from 1 to userCount
					...
					repeat
						set lcdStatus to GetItunesStatusUntillLcd("Does Not Match", "Accessing iTunes Store…", 4, "times. Check for:", 120, "intervals of", 0.25, "seconds") ------------------------Wait for iTunes to open (if closed) and the iBooks page to load
...
on verifyPage(expectedElementString, expectedElementLocation, expectedElementCount, verificationTimeout, requiresGroup)
	tell application "System Events"
		...
		(*repeat
			set lcdStatus to my GetItunesStatusUntillLcd("Does Not Match", "Accessing iTunes Store…", 4, "times. Check for:", (verificationTimeout * (1 / checkFrequency)), "intervals of", checkFrequency, "seconds")
			if lcdStatus is "Matched" then exit repeat
			delay masterDelay
		end repeat*)
...
on ProvidePaymentDetails(userFirstName, userLastName, addressStreet, addressCity, addressState, addressZip, phoneAreaCode, phoneNumber)
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		...
		repeat
			set lcdStatus to GetItunesStatusUntillLcd("Does Not Match", "Accessing iTunes Store…", 4, "times. Check for:", (netDelay * (1 / checkFrequency)), "intervals of", checkFrequency, "seconds")
			if lcdStatus is "Matched" then exit repeat
			delay masterDelay
		end repeat
